#include <linux/kernel.h>
#include <linux/init.h>
#include <linux/module.h>
#include <linux/fs.h>
#include <linux/err.h>
#include <linux/cdev.h>
#include <linux/device.h>
#include<linux/slab.h>        
#include<linux/uaccess.h>     
#include <linux/ioctl.h>

#define mem_size   1024

#define WR_DATA  _IOW('1','2',int32_t*)
#define RD_DATA  _IOR('1','3',int32_t*)


dev_t dev = 0;
static struct class *dev_class;
static struct cdev etx_cdev;
uint8_t *kernel_buffer;

static int      Sample_open(struct inode *inode, struct file *file);
static int      Sample_release(struct inode *inode, struct file *file);
static ssize_t  Sample_read(struct file *filp, char __user *buf, size_t len,loff_t * off);
static ssize_t  Sample_write(struct file *filp, const char *buf, size_t len, loff_t * off);
static long     Sample_ioctl(struct file *file, unsigned int cmd, unsigned long arg);


//File Operation Structure
static struct file_operations Sfops =
{
        .owner          = THIS_MODULE,
        .read           = Sample_read,
        .write          = Sample_write,
        .unlocked_ioctl = Sample_ioctl,
        .open           = Sample_open,
        .release        = Sample_release,
};

//Driver Open Call
static int Sample_open(struct inode *inode, struct file *file)
{
        pr_info("Driver File Open Successfully.....");
        return 0;
}

//Driver Close Call
static int Sample_release(struct inode *inode, struct file *file)
{
        pr_info("Driver File Close Successfully...");
        return 0;
}

static long Sample_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
        switch(cmd)
        {
                case WR_DATA:
                        {
                                if( copy_from_user(kernel_buffer, (char*)arg, mem_size ))
                                {
                                        pr_err("Data Write : Err!\n");
                                }
                                pr_info("Data Write : Done!\n");
                                break;
                        }
                case RD_DATA:
                        {
                                if( copy_to_user((char*)arg, kernel_buffer, mem_size) )
                                {
                                        pr_err("Data Read : Err!\n");
                                }
                                pr_info("Data Read : Done!\n");

                                break;
                        }
                default:
                        pr_info("Default\n");


        }

        return 0;

}

//Driver Read Call
static ssize_t Sample_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
        //Copy the data from the kernel space to the user-space
        if( copy_to_user(buf, kernel_buffer, mem_size) )
        {
                pr_err("Data Read : Err!\n");
        }
        pr_info("Data Read : Done!\n");
        return mem_size;
}

//Driver Write Call
static ssize_t Sample_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
        //Copy the data to kernel space from the user-space
        if( copy_from_user(kernel_buffer, buf, len) )
        {
                pr_err("Data Write : Err!\n");
        }
        pr_info("Data Write : Done!\n");
        return len;
}


//Driver initial Stage Start
static int __init Sample_init(void)
{
        /*Allocating Major number*/
        if((alloc_chrdev_region(&dev, 0, 1, "Sample_Dev")) < 0)
        {
                pr_info("Cannot allocate major number\n");
                return -1;
        }
        pr_info("Major = %d Minor = %d \n",MAJOR(dev), MINOR(dev));

        /*Creating cdev structure*/
        cdev_init(&etx_cdev,&Sfops);

        /*Adding character device to the system*/
        if((cdev_add(&etx_cdev,dev,1)) < 0)
        {
                pr_info("Cannot add the device to the system\n");
                goto r_class;
        }

        /*Creating struct class*/
        if(IS_ERR(dev_class = class_create(THIS_MODULE,"Sample_class")))
        {
                pr_info("Cannot create the struct class\n");
                goto r_class;
        }

        /*Creating device*/
        if(IS_ERR(device_create(dev_class,NULL,dev,NULL,"Sample_device")))
        {
                pr_info("Cannot create the Device 1\n");
                goto r_device;
        }

        /*Creating Physical memory*/
        if((kernel_buffer = kmalloc(mem_size , GFP_KERNEL)) == 0)
        {
                pr_info("Cannot allocate memory in kernel\n");
                goto r_device;
        }

        //strcpy(kernel_buffer, "Hello_World");

        pr_info("Device Driver Insert...Done!!!\n");
        return 0;

r_device:
        class_destroy(dev_class);
r_class:
        unregister_chrdev_region(dev,1);
        return -1;
}

//Driver Remove Stage
void __exit Sample_exit(void)
{
        kfree(kernel_buffer);
        device_destroy(dev_class,dev);
        class_destroy(dev_class);
        cdev_del(&etx_cdev);
        unregister_chrdev_region(dev, 1);
        pr_info("Device Driver Remove...Done!!!\n");
}

module_init(Sample_init);
module_exit(Sample_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Madhuprakash R");
MODULE_DESCRIPTION("Sample Driver Creation");
MODULE_VERSION("0.01");
